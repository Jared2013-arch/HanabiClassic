package cn.hanabi.gui.newStyle.clickui;

import cn.hanabi.Hanabi;
import cn.hanabi.gui.common.font.noway.ttfr.HFontRenderer;
import cn.hanabi.gui.newStyle.clickui.impl.BoolValue;
import cn.hanabi.gui.newStyle.clickui.impl.DoubleValue;
import cn.hanabi.gui.newStyle.clickui.impl.ModeValue;
import cn.hanabi.gui.newStyle.clickui.impl.Panel;
import cn.hanabi.gui.newStyle.clickui.misc.ClickEffect;
...
public class ClickUINew extends GuiScreen {
    //Panel
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float par) {
        ...
        RenderUtil.drawRoundedRect(width / 2 - 60, 2, width / 2 + 60, 18, 1, new Color(searchHover ? 255 : 245, searchHover ? 255 : 245, searchHover ? 255 : 245).getRGB());
        GL11.glPushMatrix();
        GL11.glEnable(3089);
        ...
        RenderUtil.drawRoundedRect(width / 2 + 80, 2, width / 2 + 130, 18, 1, new Color(resetHover ? 255 : 245, resetHover ? 255 : 245, resetHover ? 255 : 245).getRGB());
        ...
        if (animatranslate.getY() > 0) {
            ...
            RenderUtil.drawRoundRect(startX, startY, startX + 240, startY + 280, 1, new Color(255, 255, 255, 255).getRGB());
            if (currentMod != null) {
                //画Value
    ...
    public void drawValue(float mouseX, float mouseY, float startX, float startY) {
        ...
        //标题
        Hanabi.INSTANCE.fontManager.usans25.drawString(currentMod.getName(), startX + 15, startY + 12, new Color(124, 124, 124, 255).getRGB());
        ...
        Hanabi.INSTANCE.fontManager.icon30.drawString(iconstr, startX + 210, startY + 12, new Color(124, 124, 124, 255).getRGB());
        ...
        if (Mouse.hasWheel() && ClickUINew.isHover(mouseX, mouseY, startX, startY, startX + 240, vstartY + 235) && ClickUINew.currentMod != null) {
            if ((ClickUINew.real > 0 && settingwheel < 0)) {
                for (int i = 0; i < 10; i++) {
                    if (!(settingwheel < 0))
                        break;
                    settingwheel += 5;
                ...
                for (int i = 0; i < 10; i++) {
                    if (!(ClickUINew.real < 0 && moduleHeight > 235 && Math.abs(settingwheel) < (moduleHeight - 235)))
                        break;
                    settingwheel -= 5;
                }
            }
