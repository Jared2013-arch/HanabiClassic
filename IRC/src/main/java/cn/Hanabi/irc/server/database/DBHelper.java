...
public class DBHelper {
    ...
    public static String register(String username, String password, String key) {
        ...
        try {
            ...
            String rank = "User";
            if (!key.isEmpty()) {
                rs = statement.executeQuery("SELECT * FROM `activekeys` WHERE `activekey` = '" + key + "'");
                while (rs.next()) {
                    rank = rs.getString(2);
                }
                if (rank.equals("User")) {
            ...
            //注册用户
            boolean reg = statement.execute("INSERT INTO `fpsmaster`.`users` (`username`, `password_md5`, `rank`) VALUES ('" + username + "', '" + password + "', '" + rank + "');");
